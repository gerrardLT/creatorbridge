...
import { User, Wallet, Copy, ExternalLink, ArrowUpRight, ArrowDownLeft, Layers, Activity, FileCheck, Check } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'assets' | 'licenses' | 'history'>('assets');
  ...
  const fetchLicenses = async () => {
    ...
    try {
      const res = await fetch(`/api/license?buyerId=${user.id}`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10 pb-20">
        ...
        <div className="flex space-x-8 border-b border-white/10 mb-8 px-4">
          <button
            ...
            className={`pb-4 px-2 text-sm font-bold uppercase tracking-wider border-b-2 transition-all flex items-center gap-2 ${activeTab === 'assets' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            <Layers size={16} />
            My Assets
          </button>
          <button
            ...
            className={`pb-4 px-2 text-sm font-bold uppercase tracking-wider border-b-2 transition-all flex items-center gap-2 ${activeTab === 'licenses' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            <FileCheck size={16} />
            My Licenses
          </button>
          <button
            ...
            className={`pb-4 px-2 text-sm font-bold uppercase tracking-wider border-b-2 transition-all flex items-center gap-2 ${activeTab === 'history' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
          >
            <Activity size={16} />
            Transactions
          </button>
          ...
          {activeTab === 'licenses' && (
            <div className="animate-fadeInUp">
              {isLoadingLicenses ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="bg-zinc-900/50 rounded-2xl p-4 animate-pulse">
                      <div className="aspect-square bg-zinc-800 rounded-xl mb-4"></div>
                ...
                <div className="text-center py-20 bg-zinc-900/30 rounded-3xl border border-dashed border-zinc-800">
                  <FileCheck size={48} className="mx-auto text-zinc-600 mb-4" />
                  <p className="text-zinc-500 mb-6">No licenses purchased yet</p>
                  <Link href="/explore" className="px-8 py-3 bg-white/5 border border-white/10 rounded-full hover:bg-white/10 text-white font-bold transition-all inline-block">
                    Explore Marketplace
                  </Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                  {licenses.map((license) => (
                    <Link
                      key={license.id}
                      href={`/ip/${license.ipAsset.id}`}
                      className="group bg-zinc-900/60 backdrop-blur-md rounded-2xl overflow-hidden border border-white/5 hover:border-indigo-500/30 transition-all hover:-translate-y-1"
                    >
                      <div className="relative aspect-square overflow-hidden">
                        <img
                          src={license.ipAsset.imageUrl}
                          alt={license.ipAsset.title}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                        />
                        <div className="absolute top-3 right-3">
                          <LicenseBadge licenseType={license.ipAsset.licenseType} size="sm" />
                        </div>
                        <div className="absolute bottom-3 left-3">
                          <span className="bg-green-500/20 backdrop-blur-md border border-green-500/30 text-green-400 text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1">
                            <FileCheck size={12} />
                            Licensed
                          </span>
                        </div>
                      ...
                      <div className="p-4">
                        <h3 className="font-bold text-white group-hover:text-indigo-400 transition-colors mb-2 truncate">
                          {license.ipAsset.title}
                        </h3>
                        <div className="flex items-center justify-between text-xs text-zinc-500">
                          <span>Purchased {new Date(license.purchasedAt).toLocaleDateString()}</span>
                          {license.txHash && (
                            <a
                              href={`https://aeneid.storyscan.xyz/tx/${license.txHash}`}
                              target="_blank"
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-white/5">
                      ...
                      <tr key={tx.id} className="hover:bg-white/5 transition-colors group">
                        <td className="px-6 py-4">
                          <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${tx.type === 'REGISTER' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : tx.type === 'PURCHASE' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-green-500/10 text-green-400 border-green-500/20'}`}>
                            ...
                            {tx.type === 'REGISTER' ? 'Register' : tx.type === 'PURCHASE' ? 'Purchase' : 'Sale'}
                          </div>
                        </td>
                    ...
                    {myTransactions.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-6 py-20 text-center text-zinc-600">No transactions yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
